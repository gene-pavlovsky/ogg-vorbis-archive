...
#
# dist: ova distribution generation script.
...
{
  sed -i 's/^overwrite_config=.*/overwrite_config=no/' install
  sed -i 's/^cddb_submit_user_email=.*/cddb_submit_user_email=""/' etc/cddb.conf
  sed -i 's/^enable_debug=.*/enable_debug=no/' etc/global.conf
}
